...
[NOTE]
====
Alternativamente, puede generar un archivo CSR utilizando una herramienta como OpenSSL y puede saltar a <<Paso 2: Envíe los archivos CSR>>.
...
** *Controlador A direcciones IP alternativas* -- Si el nombre común es una dirección IP, opcionalmente puede ingresar cualquier dirección IP adicional o alias para el controlador A. Para múltiples entradas, use un formato delimitado por comas.
** * Controlador de nombres DNS alternativos * -- Si el nombre común es un nombre DNS, ingrese cualquier nombre DNS adicional para el controlador A. Para varias entradas, use un formato delimitado por comas. Si no hay nombres DNS alternativos, pero especificó un nombre DNS en el primer campo, copie ese nombre aquí. El nombre DNS no puede comenzar con un comodín. Si la cabina de almacenamiento sólo tiene una controladora, el botón *Finalizar* estará disponible.
+
