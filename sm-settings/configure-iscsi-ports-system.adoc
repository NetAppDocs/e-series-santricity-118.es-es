...
.Antes de empezar
* La controladora debe incluir puertos iSCSI; de lo contrario, la configuración de iSCSI no está disponible.
...
El puerto de escucha es el número de puerto TCP que utiliza la controladora para escuchar inicios de sesión iSCSI provenientes de iniciadores iSCSI del host. El puerto de escucha predeterminado es 3260. Debe introducir 3260 o un valor entre 49 49152 y 65 65535.
...
Especificar manualmente la configuración estática
 a| 
Seleccione esta opción y después introduzca una dirección estática en los campos. (Si lo desea, puede cortar y pegar las direcciones en cada campo.) En el caso de IPv4, incluya la máscara de subred y la puerta de enlace. En el caso de IPv6, incluya la dirección IP enrutable y la dirección IP del enrutador.
